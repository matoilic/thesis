...
{  
    std::cout << "loadShader()" << std::flush << std::endl;
...
GLuint glUtils::buildShader(string shaderFile, 
                            GLenum shaderType)
{  
    std::cout << "buildShader()" << std::flush << std::endl;
...
GLuint glUtils::buildProgram(GLuint vertShaderID, 
                             GLuint fragShaderID)
{  
    std::cout << "buildProgram()" << std::flush << std::endl;
...
GLuint glUtils::buildVBO(void*   dataPointer, 
                         ...
                         GLuint  usageTypeGL)
{  
    std::cout << "buildVBO()" << std::flush << std::endl;

   // Generate a buffer id
   GLuint vboID;
   glGenBuffers(1, &vboID);
   std::cout << "after glGenBuffers()" << std::flush << std::endl;
   ...
   std::cout << "after glBindBuffer()" << std::flush << std::endl;
   ...
   std::cout << "after int bufSize()" << std::flush << std::endl;
   ...
   std::cout << "after glBufferData()" << std::flush << std::endl;
...
{  
   ...
   if (size.width  > (GLuint)maxSize || size.height > (GLuint)maxSize)
   ...
   glTexImage2D(GL_TEXTURE_2D,   // target texture type 1D, 2D or 3D
                ...
                GL_BGR,    // data format: e.g. GL_RGBA, see spec.
