...
void MSAC::fillDataContainers(std::vector<LineSegment> &lineSegments)
{
	...
	for (int i=0; i<numLines; i++)
	{
        ...
        Point p1 = lineSegments[i].start;
        Point p2 = lineSegments[i].end;
...
{	
	...
	for(int vpNum=0; vpNum < numVps; vpNum++)
	{
        ...
        std::vector<LineSegment> lineSegmentsCurrent;
		for(int i=0; i<numLines; i++)
		{
...
void MSAC::drawCS(cv::Mat &im, std::vector<std::vector<LineSegment>> &lineSegmentsClusters, std::vector<cv::Mat> &vps)
{
	...
	for(unsigned int c=0; c<lineSegmentsClusters.size(); c++)
	{
		...
		{
            Point pt1 = lineSegmentsClusters[c][i].start;
            Point pt2 = lineSegmentsClusters[c][i].end;
